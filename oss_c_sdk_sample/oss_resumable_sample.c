...
void percentage1(int64_t consumed_bytes, int64_t total_bytes)
{
...
void resumable_download_with_resumable()
{
    ...
    headers = aos_table_make(p, 0);
    aos_str_set(&bucket, BUCKET_NAME);
    aos_str_set(&object, "dw10.1.exe");
    aos_str_set(&filename, "local_big_file_2.zip");

    // download
    clt_params = oss_create_resumable_clt_params_content(p, 1024 * 1000, 3, AOS_TRUE, NULL);
    s = oss_resumable_download_file(options, &bucket, &object, &filename, headers, NULL, 
        clt_params, percentage1, &resp_headers);
